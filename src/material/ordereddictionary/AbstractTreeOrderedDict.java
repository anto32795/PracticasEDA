...
abstract public class AbstractTreeOrderedDict<K, V> implements
    ...
    public Iterable<Entry<K, V>> findRange(K minkey, K maxkey) {
        checkKey(minkey);
        checkKey(maxkey);
        Entry<K,V> min = this.find(minkey);
        Entry<K,V> max = this.find(maxkey);
        Iterable<Position<Entry<K,V>>> lista = this.bsTree.findRange(min,max);
        List<Entry<K,V>> lista_rtn = new LinkedList<>();
        for(Position<Entry<K,V>> pos: lista){
            lista_rtn.add(pos.getElement());
        }
        return lista_rtn;
    }
