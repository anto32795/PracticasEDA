...
//    }
	...
	public Position<E> first() throws RuntimeException {
		if(this.size() == 0){
			throw new RuntimeException("No elements in the dictionary");
		}
		else if(this.size() == 1){
			return this.binTree.root();
		}
		Position<E> node = this.binTree.left(this.binTree.root()); //left node of root
		while(this.binTree.hasLeft(node)){
			node = binTree.left(node);
		}
		return node;
	}
	...
	public Position<E> last() throws RuntimeException {
		...
		Position<E> node = this.binTree.right(this.binTree.root()); //left node of root
		while(this.binTree.hasRight(node)){
			node = binTree.right(node);
		}
		return node;
	}
	public Iterable<Position<E>> successors(Position<E> pos){
		List<Position<E>> rtn = new LinkedList<>();
		BinaryTree<E> subtree = binTree.subTree(binTree.right(pos));
		if(subtree.size() == 0) return rtn;
		Iterator<Position<E>> it = subtree.iterator();
		while(it.hasNext()){
			Position<E> n = it.next();
			rtn.add(n);
		}
		return rtn;
	}
    ...
    public Iterable<Position<E>> findRange(E minValue, E maxValue) throws RuntimeException{
		Iterator<Position<E>> it = new InorderBinaryTreeIterator<>(this.binTree);
		Comparator<E> cmp = this.comparator;
		List<Position<E>> lista_rtn = new LinkedList<>();
		Position<E> node;
		while(it.hasNext()){
			node = it.next();
			if(cmp.compare(node.getElement(), maxValue) > 0)
				break;
			else if(cmp.compare(node.getElement(), minValue) >= 0 && cmp.compare(node.getElement(), maxValue) <= 0){
				lista_rtn.add(node);
			}
		}
		return lista_rtn;
    ...
    public Iterable<Position<E>> predecessors(Position<E> pos){
		List<Position<E>> rtn = new LinkedList<>();
		BinaryTree<E> subtree = binTree.subTree(binTree.left(pos));
		if(subtree.size() == 0) return rtn;
		Iterator<Position<E>> it = subtree.iterator();
		while(it.hasNext()){
			Position<E> n = it.next();
